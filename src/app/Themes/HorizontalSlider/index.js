...
export default class HorizontalSlider extends PureComponent {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div className="tb_hs_post_container">
        <Splide hasTrack={false} className="tb_hs_post_slider" options={settings} ref={this.myRef}>
          <SplideTrack>
            {
              ((postData && postData.length > 0)) && postData.map((item, index) => {
                const cardData = completeDataObject[item];
                return <SplideSlide style={{ margin: 0, padding: 0 }}><Card ownerId={wall.Wall.owner} itemData={cardData} key={`key_${index}_${parseInt(item.id) + 1}`} itemIndex={index} personalization={wall.Personalization} ThemeRule={wall.ThemeRule} clickToShowPopUp={clickToShowPopUp} wallID={wall.Wall.id} onClickToCTA={onClickToCTA} />
                </SplideSlide>
              })}
          </SplideTrack>
          <div className="splide__arrows splide__arrows--ltr tb_hs_arrow_wrapper_">
            <div className="tb_hs_arrow splide__arrow splide__arrow--prev tb_hs_arrow tb_hs_arrow_left__ tb__icon tb-arrow-left-alt" onClick={this.onUpdateData} > </div>
            <div className="tb_hs_arrow splide__arrow splide__arrow--next  tb_hs_arrow_right__ tb__icon tb-arrow-right-alt"></div>
          </div>
        </Splide>
      </div>
    );
