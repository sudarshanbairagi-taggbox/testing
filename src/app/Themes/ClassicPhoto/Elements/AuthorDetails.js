...
export default class AuthorDetails extends PureComponent {
  render() {    
    ...
    return (
      <div className="tb_cp_author_wrapper">
        <div className="tb_cp_author">
            {postAuthor ||  ownerId === 100231 ?<Author author={author} network={network} authorClass={'tb_cp_author_profile'} /> : '' }

            <div className="tb_cp_author_info">
              {postAuthor ||  ownerId === 100231 ? <div className="tb_cp_authorname" style={authorNameStyle}>{author.name}</div> : ''}
              <div className="tb_cp_post_info">
                {postAuthor ||  ownerId === 100231 ? <div className="tb_cp_username" style={authorNameStyle}>{username}</div>  : '' }
                {authorSeprator ? <div className="tb_cp_seprator" style={sepratorStyle}> </div> : ''}
                {personalization.postTime ? <Time postTime={postTime} timeClass={'tb_cp_time'} authorColor={ThemeRule.authorColor} /> : '' }
              </div>
            </div>
        </div>
        <div className="tb_cp_social_">
            <Network networkClass={'tb_cp_social_ico'} network={network} ThemeRule={ThemeRule} font={font} />
        </div>
      </div>
    );
  }
