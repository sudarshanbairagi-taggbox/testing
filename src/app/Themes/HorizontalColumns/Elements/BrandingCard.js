...
export default class BrandingCard extends PureComponent {
  render() {
    const {brandingClass, personalization, sliderHeight } = this.props;
    ...
    return (
      <div className={brandingClass}
          ...
          <a className="tb_hc_post_in" href={BrandingLinks} target="_blank" style={brandingCard}
            ref={(el) => {
              if (el) {
                el.style.setProperty('background-image', `url(${brandingCardUrl})`, 'important');
                el.style.setProperty('padding-bottom', '110%', 'important');
